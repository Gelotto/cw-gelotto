...
use crate::msg::{
    ...
    ResourcesMsg, ResourcesQueryMsg,
};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Resources(msg) => match msg {
            ResourcesMsg::Open { resources } => open(ctx, resources),
            ResourcesMsg::Close { resources } => close(ctx, resources),
        },
        ExecuteMsg::Principal(msg) => match msg {
            PrincipalMsg::Allow {
                principal,
                resources,
...
pub fn query(
    ...
    let result = match msg {
        QueryMsg::Principal(msg) => match msg {
            PrincipalQueryMsg::IsAllowed {
                principal,
                resources,
            } => to_json_binary(&query::is_allowed(deps, principal, resources)?),
            PrincipalQueryMsg::HasRoles { principal, roles } => {
                to_json_binary(&query::has_roles(deps, principal, roles)?)
            },
            PrincipalQueryMsg::Resources { principal, cursor } => {
                to_json_binary(&query::get_principal_resources(ctx, principal, cursor)?)
            },
            PrincipalQueryMsg::Roles { principal, cursor } => {
                to_json_binary(&query::get_principal_roles(ctx, principal, cursor)?)
            },
        },
        QueryMsg::Resources(msg) => match msg {
            ResourcesQueryMsg::Get {
                principal,
                path,
                cursor,
            } => to_json_binary(&query::get_resource(ctx, path, principal, cursor)?),
        },
    }?;
    Ok(result)
}
